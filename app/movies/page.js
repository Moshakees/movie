...
export default async function MoviesPage() {
    ...
    const sections = categoriesData?.movies?.filters?.section?.items || [];
    const categories = categoriesData?.movies?.filters?.category?.items || [];
    ...
    return (
        <main>
            ...
            <div className="container-px row-container" style={{ marginTop: '100px' }}>
                <h1 style={{ color: 'white', marginBottom: '40px', fontSize: '2rem' }}>أفلام</h1>
                ...
                <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '20px', marginBottom: '30px', scrollbarWidth: 'none' }}>
                    {sections.map(sec => (
                        <Link key={`tag-sec-${sec.id}`} href={`/discover?type=movies&section=${sec.id}&title=${encodeURIComponent('أفلام ' + sec.name)}`} style={{ background: '#222', padding: '8px 16px', borderRadius: '20px', color: 'white', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
                            {sec.name}
                        </Link>
                    ))}
                    {categories.map(cat => (
                        <Link key={`tag-cat-${cat.id}`} href={`/discover?type=movies&category=${cat.id}&title=${encodeURIComponent('أفلام ' + cat.name)}`} style={{ background: '#222', padding: '8px 16px', borderRadius: '20px', color: 'white', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
                            {cat.name}
                        </Link>
                    ))}
                </div>
