...
export default async function TVPage() {
    ...
    const sections = categoriesData?.shows?.filters?.section?.items || [];
    const categories = categoriesData?.shows?.filters?.category?.items || [];
    ...
    return (
        <main>
            ...
            <div className="container-px row-container" style={{ marginTop: '100px' }}>
                <h1 style={{ color: 'white', marginBottom: '40px', fontSize: '2rem' }}>تليفزيون</h1>
                ...
                <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '20px', marginBottom: '30px', scrollbarWidth: 'none' }}>
                    {sections.map(sec => (
                        <Link key={`tag-sec-${sec.id}`} href={`/discover?type=shows&section=${sec.id}&title=${encodeURIComponent(sec.name)}`} style={{ background: '#222', padding: '8px 16px', borderRadius: '20px', color: 'white', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
                            {sec.name}
                        </Link>
                    ))}
                    {categories.map(cat => (
                        <Link key={`tag-cat-${cat.id}`} href={`/discover?type=shows&category=${cat.id}&title=${encodeURIComponent(cat.name)}`} style={{ background: '#222', padding: '8px 16px', borderRadius: '20px', color: 'white', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
                            {cat.name}
                        </Link>
                    ))}
                </div>
