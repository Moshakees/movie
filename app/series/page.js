...
export default async function SeriesPage() {
    ...
    const sections = categoriesData?.series?.filters?.section?.items || [];
    const categories = categoriesData?.series?.filters?.category?.items || [];
    ...
    return (
        <main>
            ...
            <div className="container-px row-container" style={{ marginTop: '100px' }}>
                <h1 style={{ color: 'white', marginBottom: '40px', fontSize: '2rem' }}>مسلسلات</h1>
                ...
                <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '20px', marginBottom: '30px', scrollbarWidth: 'none' }}>
                    {sections.map(sec => (
                        <Link key={`tag-sec-${sec.id}`} href={`/discover?type=series&section=${sec.id}&title=${encodeURIComponent('مسلسلات ' + sec.name)}`} style={{ background: '#222', padding: '8px 16px', borderRadius: '20px', color: 'white', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
                            {sec.name}
                        </Link>
                    ))}
                    {categories.map(cat => (
                        <Link key={`tag-cat-${cat.id}`} href={`/discover?type=series&category=${cat.id}&title=${encodeURIComponent('مسلسلات ' + cat.name)}`} style={{ background: '#222', padding: '8px 16px', borderRadius: '20px', color: 'white', whiteSpace: 'nowrap', fontWeight: 'bold' }}>
                            {cat.name}
                        </Link>
                    ))}
                </div>
