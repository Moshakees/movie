...
/**
 * Akoam API - Advanced Pro Edition (v3.7)
 * Author: Manus AI
 * Features: Full Story, Featured Section, Latest Content, Search, Details, Auto-Clean Data
 * Updated: 2026-03-07
 */
...
class AkoamAPI {
    ...
    public function getLatestHome() {
        ...
        $sections = [];
        // Extract all sections like "أحدث الأفلام", "أحدث المسلسلات", etc.
            ...
            foreach ($filters_map as $filter_name => $display_label) {
                if (preg_match('/<select[^>]*name="' . $filter_name . '"[^>]*>(.*?)<\/select>/s', $html, $m)) {
                    ...
                    for ($i = 0; $i < count($options[1]); $i++) {
                        $id = $options[1][$i];
                        $title = $this->cleanText($options[2][$i]);
    ...
    public function getCategoryContent($type, $filters = [], $allPages = false) {
        $baseUrl = $this->baseUrl . '/' . $type;
        $query = http_build_query($filters);
        $url = $baseUrl . ($query ? '?' . $query : '');
        $allResults = [];
        $currentPage = 1;
        $maxPages = $allPages ? 50 : 1; 

        do {
            $pageUrl = $url . (strpos($url, '?') !== false ? '&' : '?') . 'page=' . $currentPage;
            $html = $this->request($pageUrl);
            if (!$html) break;
            $pageResults = $this->parseList($html);
            if (empty($pageResults)) break;
            $allResults = array_merge($allResults, $pageResults);
            if (!$allPages || !preg_match('/page=' . ($currentPage + 1) . '[^"]*"/', $html)) break;
            $currentPage++;
        } while ($currentPage <= $maxPages);

        return ['type' => $type, 'filters' => $filters, 'total_pages_fetched' => $currentPage, 'count' => count($allResults), 'results' => $allResults, 'status' => 'success'];
    }
    ...
    public function getDetails($url) {
        ...
        foreach ($m[1] as $item) {
            ...
            } elseif (preg_match('/(\d+\s*\/\s*\d+)/', $cleanItem, $rating)) $metadata['التقييم'] = $rating[1];
            else if (!empty($cleanItem)) $metadata[] = $cleanItem;
        }
        ...
        if (strpos($url, '/series/') !== false || strpos($url, '/show/') !== false || $isEpisode) {
            ...
            $data['episodes'] = $this->extractAllEpisodes($mainPageHtml, $url);
    ...
    private function extractAllEpisodes($html, $seriesUrl) {
        $episodes = []; $seenUrls = [];
    ...
    private function resolveGoLink($goUrl) {
        ...
        if ($html && preg_match('/href="(https:\/\/ak\.sv\/download\/.*?)"/', $html, $m)) {
            ...
            if ($dlHtml && preg_match('/href="(https:\/\/[^"]+\.(mp4|mkv|avi|mov|flv|webm)[^"]*)"/i', $dlHtml, $dl)) {
                return ['source' => $goUrl, 'direct_url' => $this->cleanOutputUrl($dl[1]), 'quality' => $this->extractQuality($dl[1])];
            }
        }
        return null;
    }
...
if (basename(__FILE__) == basename($_SERVER['SCRIPT_FILENAME'])) {
    ...
    switch ($action) {
        case 'featured': echo json_encode($api->getFeatured(), $json_flags); break;
        case 'latest': echo json_encode($api->getLatestHome(), $json_flags); break;
        case 'categories': echo json_encode(['status' => 'success', 'data' => $api->getAllCategories()], $json_flags); break;
        case 'category_content':
            $type = $_GET['type'] ?? 'movies';
            $allPages = isset($_GET['all']) && $_GET['all'] == 'true';
            $filters = [];
            foreach (['section', 'category', 'quality', 'language', 'year'] as $f) if (isset($_GET[$f])) $filters[$f] = $_GET[$f];
            echo json_encode($api->getCategoryContent($type, $filters, $allPages), $json_flags);
            break;
        case 'search': echo json_encode($api->search($_GET['q'] ?? ''), $json_flags); break;
        case 'details': echo json_encode($api->getDetails($_GET['url'] ?? ''), $json_flags); break;
        case 'episodes':
            $url = $_GET['url'] ?? '';
            $details = $api->getDetails($url);
            echo json_encode(['title' => $details['title'] ?? 'Unknown', 'episodes' => $details['episodes'] ?? [], 'status' => 'success'], $json_flags);
            break;
        default:
            $protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http";
            $full_url = "$protocol://$_SERVER[HTTP_HOST]$_SERVER[SCRIPT_NAME]";
            echo json_encode([
                'status' => 'online', 'api_name' => 'Akoam Ultimate API', 'version' => '3.7',
                'endpoints' => [
                    ...
                    ['action' => 'category_content', 'example' => "$full_url?action=category_content&type=movies&all=true"]
                ]
            ], $json_flags);
            break;
    }
